...
;;; See compiler/README-rust-passes.md for the module map.
          ...
          [(type-tfield-ftype decl-type) =>
           (lambda (ftype)
             (let ([n (literal-int-expr? rhs)])
               ;; Native Field: `field-literal-rust` (lossless above
               ;; `u64::MAX`); the JubjubScalar builtin (EmbeddedFr) has the
               ;; same `From<u64>` surface and takes that form.
               (and n (field-typed-literal-rust ftype n))))]
      ...
      (define (arith-operand-rust expr native-id-ht)
        ;; UNSIGNED `+ - *` operands are cast to the `mbits` result width by
        ;; the caller, so the operand's own `safe-cast` MUST NOT be
        ;; materialised here (that would double-cast). But its SOURCE type is
        ;; still the operand's declared type, and a compound operand whose
        ;; sub-expressions carry no per-node wrapper — a ternary whose arms
        ;; are bare literals — loses all width information otherwise and
        ;; defaults to Rust's `i32` (so `n + (c ? 3000000000 : 3000000000)`
        ;; failed to compile). Render such an operand at its own source type:
        ;; target == source, so `materialize-at-type` emits no cast, while the
        ;; arm/literal still sees a width. A bare integer-literal operand
        ;; keeps its pre-existing rendering (the caller's `as <w>` types it,
        ;; and `current-arith-suffix` covers the downcast case), preserving
        ;; byte parity for the common literal-operand shape.
        (let* ([rendered
                (if (literal-int-expr? expr)
                    (expr-rust expr native-id-ht)
                    (parameterize ([current-expr-expected-type
                                    (expr-source-type expr)])
                      (expr-rust expr native-id-ht)))]
      ...
      ;; field-typed-literal-rust: materialise an integer literal at a
      ;; `(tfield ftype)` type. Native Field takes `field-literal-rust`
      ;; (lossless above `u64::MAX`); the JubjubScalar builtin (EmbeddedFr)
      ;; has the same `From<u64>` surface and takes that form.
      (define (field-typed-literal-rust ftype n)
        (if (field-type-native? ftype)
            (field-literal-rust n)
            (format "~a::from(~au64)" (field-type-rust ftype) n)))

      ;; field-arith-operand-rust: render one operand of a FIELD `+`/`-`/`*`.
      ;; Field arithmetic has no width cast to normalise its operands, so an
      ...
      ;; (`field-typed-literal-rust`) and a `Uint` operand into
      ;; `Fr::from((x) as uN)`. A `Field`-typed operand carries no wrapper
      ...
      (define (field-arith-operand-rust expr ftype native-id-ht)
        (let ([lit (literal-int-expr? expr)])
          (cond
            [lit (field-typed-literal-rust ftype lit)]
      ...
      ;; Only reached by pure circuits that use wrapping arithmetic.
      ;;
      ...
      ;;   u128     — map_lambda_fixture (`x * 2` on Uint<64>: the result
      ...
      (define (arith-binop-rust src op type expr1 expr2 native-id-ht)
        ...
        ;;     operand to the result-type width, so materialising the
        ...
        ;;
        ;;   * FIELD (result type `(tfield ftype)`) — there is no width cast,
        ;;     so each operand is materialised from its own `safe-cast` target
        ;;     by `field-arith-operand-rust` (a bare literal -> `Fr`, a `Uint`
        ;;     -> `Fr::from((x) as uN)`).
        (let ([w (arith-result-rust-width type)])
          (cond
            [w
             (parameterize ([current-expr-expected-type #f])
            ...
            ;;
            ;; `arith-result-rust-width` also returns #f for a result wider
            ;; than u128, so the two cases MUST be distinguished here —
            ...
            [(type-tfield-ftype type) =>
             (lambda (ftype)
               (let ([rust-op (field-arith-rust-operator op)])
                 ...
                 (format "(~a) ~a (~a)"
                   (field-arith-operand-rust expr1 ftype native-id-ht) rust-op
                   (field-arith-operand-rust expr2 ftype native-id-ht))))]
            ;; Neither a known unsigned width nor a field: refuse rather than
            ;; emit `wrapping_*` against a type that may not have it. A guess
            ;; here is exactly the silent-bad-output path the field case above
            ;; spent a release demonstrating.
            [else
      ...
      (define (expr-rust expr native-id-ht)
        (nanopass-case (Ltypescript Expression) expr
          ...
          [(if ,src ,expr0 ,expr1 ,expr2)
           ...
           ;;
           ;; Each arm renders through expr-rust-typed at the conditional's
           ;; USE-POSITION type — the destination bound as
           ;; `current-expr-expected-type` — which the arm's own safe-cast is
           ;; materialised against. The arm's own wrapper is only a fallback
           ;; for when no use position declared a type. Using the use-position
           ;; type for BOTH arms is what keeps them at one Rust type: the
           ;; typer's `if` rule (analysis-passes.ss) joins the arms and
           ;; `maybe-safecast`s each narrower arm to the join, but an element
           ;; boundary that coerces the whole conditional (e.g. a
           ;; `Vector<N, Field>` native argument) may supply a destination
           ;; WIDER than an individual arm's wrapper — and if each arm
           ;; preferred its own narrower wrapper those Rust types would
           ;; diverge (`if c { Fr::from(1u64) } else { 0u8 }`, E0308).
           ;; Type/width correctness is inherited entirely from
           ;; rust-codegen/type-directed-coercion: this clause adds NO
           ;; arm-specific literal/width logic.
           (format "if ~a { ~a } else { ~a }"
                   ...
                   (expr-rust-typed expr1
                                    (or (current-expr-expected-type)
                                        (expr-expected-type expr1))
                                    native-id-ht)
                   (expr-rust-typed expr2
                                    (or (current-expr-expected-type)
                                        (expr-expected-type expr2))
                                    native-id-ht))]
          [(elt-ref ,src ,expr ,elt-name ,nat)
           ...
           (let ([w (uint-target-rust-width nat1)])
             (cond
               ...
               [else
                ...
                ;;
                ;; The expected type is cleared for the operand: this clause
                ;; already casts to the target width, so materialising the
                ;; typer's operand `safe-cast` too would double-cast.
      ...
      ;; the tuple; the wrapper's target and source supply the element types,
             ...
             (let loop ([ta* tuple-arg*]
               ...
               (if (null? ta*)
                   ...
                   (let* ([ta (car ta*)]
                          ...
                          ;; An explicitly cast element (`0 as Field` is a
                          ;; `safe-cast` around the literal) names its own
                          ;; destination in the wrapper; `expr-value-type`
                          ;; strips casts and knows nothing about literals.
                          [inferred (and (not te)
                                         (or (expr-expected-type inner)
                                             (expr-value-type inner)))]
                          [recur-te (or te inferred)]
                          [recur-se (or se inferred)])
                     ;; A nested tuple literal whose own type is unknown (no
                     ;; enclosing coercion, no wrapper of its own — every cast
                     ;; sits on its elements) is still an aggregate and must be
                     ;; flattened: rendering it whole gives
                     ;; `AlignedValue::from([x, 0])` with the element casts
                     ;; peeled — a Byte-aligned hash of the wrong values.
                     (if (or (type-is-aggregate? recur-te)
                             (nanopass-case (Ltypescript Expression) (expr-strip-cast inner)
                               [(tuple ,src^ ,tuple-arg^* ...) #t]
                               [else #f]))
                                   ...
                                   (format "midnight_compact_runtime::AlignedValue::from(~a)"
                                     (if (and te se)
                                         (materialize-element src te se ta
                                           (lambda (e) (expr-rust e native-id-ht)))
                                         ;; No enclosing coercion: the element's
                                         ;; own type is the expectation, so an
                                         ;; explicitly cast literal (`0 as Field`
                                         ;; is a `safe-cast` around `0`)
                                         ;; materialises as `Fr::from(0u64)`
                                         ;; rather than a bare `0` that no
                                         ;; `AlignedValue` conversion accepts.
                                         (tuple-arg-rust ta native-id-ht recur-te)))))))))))]
          [else
                   ...
                   (list
                     (format "midnight_compact_runtime::AlignedValue::from(~a)"
                       (if (and target-type source-type)
                           ...
                           ;; No enclosing coercion: render at the value's own
                           ;; type so an explicitly cast scalar literal
                           ;; materialises (see the tuple branch above).
                           (expr-rust-typed arg-expr elt-target native-id-ht)))))))]))
