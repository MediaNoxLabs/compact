    ...
    (case-lambda
      ...
      [(pathname output-directory-pathname final-pass test-hook)
       ...
       (when (and (emit-rust) (feature-zkir-v3))
         (external-errorf "--target rust does not support --feature-zkir-v3 yet; use --target rust with the default (ZKIR v2) backend"))
                          ...
                          (let ([ltypescript-ir (run-passes prepare-for-typescript-passes analyzed-ir)])
                            ...
                            (when (emit-rust)
                              ...
                              (let ([lib-rs (format "~a/contract/lib.rs"
                                                    output-directory-pathname)]
                                    ;; `shell-quote-word` (utils.ss): a bare
                                    ;; `'~a'` broke on a path containing an
                                    ;; apostrophe — the remainder became shell
                                    ;; syntax, so rustfmt silently did not
                                    ;; run, or worse.
                                    [shell-word shell-quote-word])
