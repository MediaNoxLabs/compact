        ...
        (let ([e (expr-strip-cast expr)])
          (nanopass-case (Ltypescript Expression) e
            ...
            [(if ,src ,expr0 ,expr1 ,expr2)
             ...
             ;; condition and both branches. The emitter's matching
             ;; `if` clause (expr-rust in rust-passes-emit.ss) renders a
             ;; lazy Rust `if` expression, routing the condition through
             ;; cond-rust and each branch through expr-rust, so every
             ;; part must be a shape those renderers handle. Without
             ;; this arm the `[else #f]` rejected any body containing a
             ;; ternary BEFORE emission was attempted (impure circuits'
             ;; `circuit-body-emission`, constructors via the same gate
             ;; in emit-ctor-body-or-fallback's callers), even though the
             ;; emitter could render it. Pure bodies need no walker
             ;; change — stmt-pure-body-rust doesn't gate through
             ;; expr-supported?.
