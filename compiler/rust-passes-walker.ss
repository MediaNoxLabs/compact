...
;;; body routes (constructor / impure / pure).
      ...
      (define (coerce-cmp-operand-rust expr type local-binds
        ...
        (cond
          ...
          ;; EXCEPT when the joined type is a field type, where an integer
          ;; literal can never unify with the `Fr` / `JubjubScalar` struct
          ;; and must be materialised at that type
          ;; (`field-typed-literal-rust`).
          [(literal-int-expr? expr)
           (cond
             [(type-tfield-ftype type) =>
              (lambda (ftype)
                (field-typed-literal-rust ftype (literal-int-expr? expr)))]
             [else
