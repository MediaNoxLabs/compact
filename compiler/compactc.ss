...
The following flags, if present, affect the compiler's behavior as follows:
  ...
  --target <language> selects a contract-code backend. It is repeatable, and the
    ...
    The Rust backend is not yet compatible with --feature-zkir-v3 (it targets
    the ZKIR v2 pipeline). ZKIR and proving keys are independent of this flag
    and are still generated unless --skip-zk is also set.
"))
...
(parameterize ([reset-handler abort])
  (command-line-case (command-line)
    [((flags [(--help) $ (begin (print-help) (exit))]
     ...
     ;; `--target rust` with `--feature-zkir-v3` is rejected in passes.ss
     ;; (the Rust backend targets the ZKIR v2 pipeline only).
