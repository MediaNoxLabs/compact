...
- Add a compiler-backed CI job **on `ubuntu-latest`** in `rust-runtime-test.yml` that sets up Nix **and** a Rust toolchain, runs `nix build .#compactc`, then `cargo test -p tests-e2e-rust --locked` **without** the `--skip rust_codegen_byte_parity` / `--skip rust_backend_` exclusions. The existing Linux+macOS `test` matrix keeps its `--skip` flags (neither leg has Nix) and its header/step comments are corrected so they no longer claim to run the byte-parity suite; extending the lane to the macOS matrix is deferred to a follow-up. (Self-contained Linux job first; sharing `build-compiler.yml`'s built `compactc` artifact is a later optimisation, tracked in [#23](https://github.com/MediaNoxLabs/compact/issues/23).)
- Add mechanical gate invariants so the gate holes themselves cannot reappear:
  ...
  - a gate that `cargo check`s each freshly regenerated fixture `lib.rs` against the committed fixture manifest (the `midnight-compact-runtime` path dep), independent of byte-parity, so emitted code that fails to compile is caught even where a byte comparison would not see it. The emitted `contract/Cargo.toml` is **not** used: it declares `compact-runtime = "<TypeScript-runtime version>"` while the emitted `lib.rs` uses `midnight_compact_runtime`, and that registry crate is not in `Cargo.lock`;
