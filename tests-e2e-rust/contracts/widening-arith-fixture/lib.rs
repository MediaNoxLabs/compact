...
impl<'a, D: DB> Ledger<'a, D> {
    pub fn last_area(&self) -> Result<u64, CompactError> {
...
pub mod pure_circuits {
    ...
    pub fn sum_bytes(a: u8, b: u8) -> Result<u16, CompactError> {
        Ok(((a) as u16).wrapping_add((b) as u16))
    }

    pub fn age_threshold_days(years: u8) -> Result<u32, CompactError> {
        Ok(((years) as u32).wrapping_mul((365) as u32))
    }

    pub fn product_bytes(a: u8, b: u8) -> Result<u16, CompactError> {
        Ok(((a) as u16).wrapping_mul((b) as u16))
    }

    pub fn area_of(w: u16, h: u16) -> Result<u32, CompactError> {
        Ok(((w) as u32).wrapping_mul((h) as u32))
    }
}
