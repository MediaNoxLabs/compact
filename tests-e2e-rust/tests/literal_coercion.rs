...
fn every_coerced_position_round_trips() {
    ...
    // Compiler 0.34 no longer coerces a `Vector<2, Uint<8>>` argument into a
    // `Vector<2, Field>` implicitly, so these two probes now hash the `Uint<8>`
    // vector at its own type: the whole var-ref argument (and the nested
    // `[w]`) must still bind once and flatten to Byte-aligned leaves rather
    // than wrap the array in `AlignedValue::from` (which has no `From` impl).
    let expected_u8_var_ref_hash = midnight_compact_runtime::std_lib::persistent_hash_aligned(&[
        AlignedValue::from(5u8),
        AlignedValue::from(0u8),
    ]);
    assert_eq!(
        pure_circuits::hash_var_ref_vector_arg([5u8, 0]).expect("var-ref vector arg"),
        expected_u8_var_ref_hash
    );
    assert_eq!(
