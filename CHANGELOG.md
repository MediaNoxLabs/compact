...
- **`--target rust` emitted comparisons whose operands mixed minimal Rust
  widths, failing `cargo build` with `E0308`.** Compact's typer tracks value
  ranges, not declared widths: `q * 4` on `q: Uint<32>` types as
  `Uint<0..17179869181>`, which the rust backend deliberately renders at
  `u64` (range fidelity — deliberately NOT narrowed back to `u32`). When such
  a range-widened expression met a `Uint<32>`-ranged one inside a binary
  operation, the typer wrapped the narrower operand in a `safe-cast` to the
  joined range — and the emitter peeled that wrapper silently, so the
  operands met as `u64` vs `u32` and the generated crate did not compile.

  The gap surfaced via the digital-passport dogfood fixture
  (`add-digital-passport-dogfood-fixture`): `cargo build -p
  compact-contract-digital-passport-credential` failed with 13 `E0308`s, all
  rooted in `assertCivilDateMatchesEpochDays` and the age predicate —
  every one a typer-legal mixed-width site (`yearAdjustedQuotient4 * 4 <=
  yearAdjusted`, its underflow guard, the chained `< q*d + d`, and the
  `month >= 3 ? month - 3 : month + 9` branch join whose arms ranged u32
  vs u64).

  The fix materialises the typer's widening annotation at the boundaries
  where widths must agree: comparison operands (`<= >= < > == !=`) and
  conditional-expression branches render a wrapped narrower operand as
  `(operand) as <wider>` — a lossless zero-extension — while the wider side
  keeps its own minimal-width rendering (`q * 4` stays `u64`). Bare integer
  literals still peel silently (Rust infers their width), so uniform-width
  and literal-widening sites are byte-unchanged. The `+ - *` routes already
  emitted same-width operands via the `mbits` result-width cast and needed
  no change; the constructor body walker additionally learned to skip the
  declaration-only `const` statements the typer emits when a constructor's
  const right-hand side lifts its own temps (`const diff = base - q * 4;`).

  Gates: new `examples/mixed_width_operand_fixture.compact` (per-operator
  mixed-width circuits, the exact dogfood shape, guarded subtraction,
  impure + constructor routes) with executing test
  `tests-e2e-rust/tests/mixed_width_operand_fixture.rs` driving values past
  the 2^32 boundary so a truncating or wrongly-directed cast cannot pass by
  merely compiling; full FIXTURES corpus regenerated byte-identically
  outside genuine mixed-width sites; the dogfood crate now builds with zero
  `E0308`.
