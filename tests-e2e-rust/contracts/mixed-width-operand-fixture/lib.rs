    ...
    W: Witnesses<PS>,
{
    ...
    pub fn record_pinned(
        ...
        let _ = pure_circuits::assert_quotient_pinned(quotient4, year, month)?;
        let tmp = 1u16;
        let ops = OpProgramVerify::<DefaultDB>::new()
...
pub mod pure_circuits {
    ...
    pub fn assert_quotient_pinned(
        quotient4: u32,
        year: u32,
        month: u32,
    ) -> Result<(), CompactError> {
        let year_adjusted = if (month <= 2) {
            {
                compact_assert!((year >= 1), "result of subtraction would be negative");
                ((year) as u32).wrapping_sub((1) as u32)
            }
        } else {
            year
        };
        let t = ((quotient4) as u64).wrapping_mul((4) as u64);
        compact_assert!(
            ((t <= ((year_adjusted) as u64))
                && (((year_adjusted) as u64)
                    < ((((quotient4) as u64).wrapping_mul((4) as u64)) as u64)
                        .wrapping_add((4) as u64))),
            "quotient for 4 is invalid"
        );
        let remainder4 = {
            let t_0 = ((quotient4) as u64).wrapping_mul((4) as u64);
            {
                compact_assert!(
                    (((year_adjusted) as u64) >= t_0),
                    "result of subtraction would be negative"
                );
                ((year_adjusted) as u32).wrapping_sub((t_0) as u32)
            }
        };
        compact_assert!((remainder4 <= 3), "remainder for 4 is invalid");
        Ok(())
    }
