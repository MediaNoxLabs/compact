...
;;; See compiler/README-rust-passes.md for the module map.
      ...
      (define (materialize-scalar src target-type source-type expr render-inner)
        (nanopass-case (Ltypescript Type) (type-strip-alias target-type)
          [(tfield ,src^ ,ftype)
           ;; A literal materialises at the field type itself (native `Fr`
           ;; or the JubjubScalar builtin). A `Uint` value is coerced only
           ;; into the native field: the typer never widens a Uint into a
           ;; JubjubScalar implicitly (that is an explicit `cast-to-field`).
           (let ([lit (literal-int-expr? expr)])
             (cond
               [lit (field-typed-literal-rust ftype lit)]
               [(not (field-type-native? ftype)) #f]
               [else
      ...
      (define stdlib-struct-mappings
        `((Maybe
          ...
          (JubjubSchnorrSignature
            ,(lambda (elt-name* type*) "midnight_compact_runtime::JubjubSchnorrSignature")
            #t)))
      ...
      (define stdlib-circuit-mappings
        `((some
          ...
          ;; emitter.
          (jubjubSchnorrVerify
            ,(lambda (cdefn) "midnight_compact_runtime::std_lib::jubjub_schnorr_verify"))))
