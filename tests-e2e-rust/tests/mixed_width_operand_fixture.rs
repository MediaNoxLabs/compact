...
// the digital-passport dogfood shape (narrow ternary join pinned by a
// chained assert), in an impure inline equality, and in the constructor.
//
...
/// The dogfood shape: cascading consts, narrow ternary join, chained
/// assert (`q*d <= n < q*d + d`), and the guarded remainder — driven on
/// the March-onward branch (month >= 3, no year adjustment) and the
/// January/February branch (year - 1), plus the reject side one product
/// step past the year.
#[test]
fn assert_quotient_pinned_each_branch() {
    // month >= 3: yearAdjusted = year = Uint<32>::MAX, pinned by
    // quotient4 = (2^32 - 1 - 3) / 4.
    pure_circuits::assert_quotient_pinned(CAPTURE_Q, u32::MAX, 4)
        .expect("top-of-range quotient must pin");
    // month <= 2: yearAdjusted = year - 1 = 2025 = 506 * 4 + 1.
    pure_circuits::assert_quotient_pinned(506, 2026, 1).expect("March-based adjustment must pin");
    let err = pure_circuits::assert_quotient_pinned(1_073_741_824, u32::MAX, 4)
        .expect_err("4294967296 <= 4294967295 must fail");
    assert!(
        matches!(err, CompactError::AssertionFailed(ref m) if m == "quotient for 4 is invalid"),
        "expected the quotient assert, got {err:?}"
    );
}
...
fn impure_circuits_forward_and_write() {
    ...
    let after_pin = contract
        ...
        .expect("valid quotient must commit");
    ...
    let err = contract
        ...
        .err()
        .expect("invalid quotient must propagate the assert");
    assert!(
        matches!(err, CompactError::AssertionFailed(ref m) if m == "quotient for 4 is invalid"),
        "expected the quotient assert, got {err:?}"
    );
